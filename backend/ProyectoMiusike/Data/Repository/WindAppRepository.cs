...
namespace ProyectoMiusike.Data.Repository
{
    public class WindAppRepository : IWindAppRepository
    {
        ...
        public async Task<IEnumerable<CacionEntity>> GetAllCanciones(int orderBy)
        {
            IQueryable<CacionEntity> query = windDBContext.Cansiones;
            query = query.AsNoTracking();
        switch (orderBy)
        {
            case 1:
                query = query.OrderByDescending(a => a.Votacion);
                break;
            case 2:
                query = query.OrderByDescending(a => a.Ventas);
                break;
            default:
                break;
        }
